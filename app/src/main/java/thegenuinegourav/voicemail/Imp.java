        ...
        tts = new TextToSpeech(this, new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    speak("Say from address of the mailer whose message you want to save as favourite");
                ...
                if(result.get(0).equals("cancel"))
                {
                    ...
                    switch (noc) {
                        case 1:
                            ...
                            f = f + "@gmail.com";
                            ft.setText(f);
                            speak("Say subject of the mail whose message you want to  mark as important");
                            break;
                        case 2:
                            String s;
                            s = result.get(0);
                            st.setText(s);
                            speak("Please Confirm the important details \n from : " + ft.getText().toString() + "\nSubject : " + st.getText().toString() + "\nSpeak Yes to confirm or no to reenter");
                            break;
                        case 3:
                            if(result.get(0).equals("yes"))
                            {
                                ...
                                System.out.println("FIMP    :  " +ftrash);
                                System.out.println("SIMP    :  " +strash);

                                DatabaseHelper db1 = new DatabaseHelper(this);
                                db1.impMail(ftrash,strash);
                                speak("Message saved as Important");
                                Intent intent = new Intent(Imp.this, Options.class);
                                startActivity(intent);
                            }
                            else if(result.get(0).equals("no")) {
                                Intent intent = new Intent(Imp.this, Imp.class);
                                startActivity(intent);
                            }
                            else
                            {
                                speak("Failed to recognize voice command");
                                Intent intent = new Intent(Imp.this, Trash1.class);
                                startActivity(intent);
                            }
                    }
                }
